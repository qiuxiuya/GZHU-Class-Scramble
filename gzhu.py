...
class GZHU(object):
    ...
    def tuike(self, daixuan_data: dict):
        """退课"""
        print('正在退课:{}'.format(daixuan_data['course_name']))
        tuike_url = 'http://jwxt.gzhu.edu.cn/jwglxt/xsxk/zzxkyzbjk_xkTkZzxkYzb.html?gnmkdm=N253512' \
                    '&su={}'.format(self.username)
        kcmc = '({kch}){cn}+-+{xf}+学分'.format(
            kch=daixuan_data['kch_id'], cn=daixuan_data['course_name'],
            xf=daixuan_data['xf'])
        data = {
            'do_jxb_id': daixuan_data['do_jxb_id'],
            'kch_id': daixuan_data['kch_id'],
            'kcmc': kcmc,
            'kklxdm': daixuan_data['kklxdm'],
            'njdm_id': self.njdmId,
        ...
        resp = self.client.post(url=tuike_url, data=data, headers=self.base_headers, timeout=None)
        try:
            result = json.loads(resp.text)
            if result.get('flag') == '1':
                return True, '退课成功'
            else:
                return False, result.get('msg', '退课失败')
        except Exception as e:
            print('TUIKE ERROR:{}'.format(e))
            return False, '退课请求异常'
